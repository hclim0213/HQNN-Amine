...
def setup_param_dict(args, data_type):
    ...
    param_dict = {
        'input_size': 84, 'hidden_size': 512, 'edge_size': 12,
        'output_size': 1, 'dropout': 0.1, 'steps': 1,
        'batch_size': 128, 'lr': 1e-3, 'epochs': args.epochs,
        'type': type_dict[data_type], 'n_qubits': args.n_qubits,
        'n_layers': args.n_layers, 'model_name': args.model_name,
        'q_device': args.q_device, 'mode': args.mode,
    }
...
def build_model(cv, param_dict, device):
    if param_dict['mode'] == 'scratch':
        graph = HQSc_DMPNN(
            input_size=param_dict['input_size'], output_size=param_dict['output_size'],
            hidden_size=param_dict['hidden_size'], edge_size=param_dict['edge_size'],
            steps=param_dict['steps'], n_qubits=param_dict['n_qubits'],
            dropout=param_dict['dropout'], type=param_dict['type']
        ).to(device)

        return QML_hybrid_graph(
            input_size=param_dict['input_size'], hidden_sizes=param_dict['hidden_size'],
            output_size=param_dict['output_size'], dropout=param_dict['dropout'],
            n_qubits=param_dict['n_qubits'], n_layers=param_dict['n_layers'],
            graph=graph, q_device=param_dict['q_device']
        ).to(device)

    elif param_dict['mode'] == 'transfer':
        pretrained_model = HQFx_DMPNN(
            input_size=param_dict['input_size'], output_size=param_dict['output_size'],
            hidden_size=param_dict['hidden_size'], edge_size=param_dict['edge_size'],
            steps=param_dict['steps'], dropout=param_dict['dropout'],
        ...
        return QML_hybrid_pretrained_graph(
            input_size=param_dict['input_size'], hidden_size=param_dict['hidden_size'],
            output_size=param_dict['output_size'], dropout=param_dict['dropout'],
            n_qubits=param_dict['n_qubits'], n_layers=param_dict['n_layers'],
            type=param_dict['type'], graph=pretrained_model,

...
from src.models.GNN import DirectedMessagePassingNetwork
...
def train_result(df_copy, ps, param_dict, device):
    ...
    for cv, (train_idx, valid_idx) in enumerate(ps.split()):
        ...
        model = DirectedMessagePassingNetwork(input_size=param_dict['input_size'], hidden_size=param_dict['hidden_size'], output_size=param_dict['output_size'], 
                                             edge_size=param_dict['edge_size'], steps=param_dict['step'], dropout=param_dict['dropout'], type=param_dict['type'])
...
if __name__=="__main__":
    parser = argparse.ArgumentParser(description='DMPNN training')
    ...
    for hidden_size in hidden_sizes:
        for step in steps:
            model_name = 'DMPNN_' + data_type
            model_name = f"{model_name}_{hidden_size}_{step}"
            
            trans_estimator = f"Hidden_size\t{hidden_size}\tStep\t{step}"

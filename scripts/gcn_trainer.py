...
from src.models.GNN import GraphConvNetwork
...
def train_result(df_copy, ps, param_dict, device):
    ...
    for cv, (train_idx, valid_idx) in enumerate(ps.split()):
        ...
        model = GraphConvNetwork(input_size=param_dict['input_size'], hidden_size=param_dict['hidden_size'], output_size=param_dict['output_size'], 
                                        num_layers=param_dict['num_layers'], dropout=param_dict['dropout'], type=param_dict['type'])
...
if __name__=="__main__":
    ...
    for hidden_size in [512, 1024, 2048]:
        for num_layers in [1, 2, 3, 4]:
            ...
            model_name = f"{model_name}_{hidden_size}_{num_layers}"
            
            trans_estimator = f"Hidden_size\t{hidden_size}\tNum_layer\t{num_layers}"
